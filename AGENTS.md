...
- 正式 appid（wx1d17bbdd182f2b02）已入库——appid 是公开标识符，可提交；自部署替换 appid 或用 project.private.config.json 覆盖；云环境用 cloud.local.ts
- 上传私钥 private.wx*.key 存放在仓库外 ../keys/ 目录，严禁移入仓库或提交；.auth/ 凭证目录同理（已被 gitignore）
