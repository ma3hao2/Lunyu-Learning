...
- 📖 **二十篇全量精读**：509 条章句，含原文、白话译文、注释解读、核心要点（中文数据 deflate 压缩后放入分包，主包保持轻量；英文压缩 blob 随主包，供首页英文模式即时取值）
- 🌐 **中英双语切换**：设置页/首页一键切换界面语言（中文 / English），章句译文、注释解读、核心要点与全部界面文案随语言切换（英文缺失逐字段回退中文，原文文言不翻译）；英文数据与 lunyu-web 共用同一翻译产线
...
- 🧪 **测试完备**：257 项 Jest 单元测试全绿（存储合并、云函数、解压器、双语搜索、i18n、每日推荐等）
